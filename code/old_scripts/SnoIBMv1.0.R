...
# Load model parameters
  ...
  fish_other.constants = fncReadConstants(parameters["fish_other", "spp"])
...
for(iter in iter.list){
  ...
  if(SA == TRUE) {parameters = fncGetParametersSA(vary_nFish = TRUE)}
...
#=== INITIALIZE HABITAT ========================================================
  ...
  if(network == "nhd1")  wq.df = wq.df[, c("pid", "rid", so.field, xlat, ylon, "ratio", "upDist", "afvFlow", "locID", "netID", width.field, ration.cols, "density")]
  if(network == "rbm")  wq.df = wq.df[, c("arcid","pid", "rid", so.field, xlat, ylon, "ratio", "upDist", "afvArea", "locID", "netID", width.field, ration.cols, "density")]
  nwq = nrow(wq.df)
...
#=== INITIALIZE FISH ===========================================================
  ...
  set.seed(iter)
  salmon.spawn.times = hist(rnorm(nrow(salmon), mean = 0, sd = parameters["salmon", "spawn.date.shape"]), breaks = (parameters["salmon", "nSpawnDays"] * 2), plot = FALSE)$counts
  ...
  # 7. GROW FISH. (calculated using the Wisconsin Bioenergetics model equations)
      ...
      if(SecondSpecies == TRUE){
        if(length(fish_other.alive.index) > 0){
          ...
          # kill off any fish that have negative weight
