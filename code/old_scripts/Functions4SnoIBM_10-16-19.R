...
#Import DHSVM Flow into SSN
fncImportQ<- function(filepath){
  
  #Flow - read in and format data from DHSVM
  Qr = read.csv(filepath, skip = 1, header = FALSE, stringsAsFactors = FALSE)
  Qr[1,]<- c(0, Qr[1,-ncol(Qr)])
  colnames(Qr) = c("Date",paste0("X", Qr[1,-1]))
  Qr = Qr[-1,]
  Qr[,2] = as.numeric(Qr[,2])
  dt1 = Qr[,1] #extract date/times
  dt2 = gsub("-", " ", dt1)
  dt3 = gsub("\\.", "/", dt2) #special code for replacing "."
  dt4 = unlist(strsplit(dt3, " ") )
  date1 = dt4[seq(1, length(dt4), 2)]
  date2 = as.Date(date1, format = '%m/%d/%Y')
  time1 = dt4[seq(2, length(dt4), 2)]
  time2 = as.numeric(substr(time1, 1, 2))
  Q = as.matrix(Qr[,-1])
  Q.df = cbind.data.frame("Date" = date2, "Time" = time2, Q)
  
  return(Q.df)
  
}
#Q.df = fncImportQ("data.in/rbm.data/Outflow.Only.csv")

#Import RBM water temperature into SSN
fncImportWT<- function(filepath){
  
#Water temp - read in and format data from RBM
Tw = read.csv(filepath, skip = 1, header = FALSE, stringsAsFactors = FALSE)
#Tw = read.csv(filepath, skip = 1, sep = " ", header = FALSE, stringsAsFactors = FALSE)
Tw[1,]<- c(0, Tw[1,-ncol(Tw)])
colnames(Tw) = c("Date",paste0("X", Tw[1,-1]))
Tw = Tw[-1,]
Tw[,2] = as.numeric(Tw[,2])
dt1 = Tw[,1] #extract date/times
dt2 = gsub("-", " ", dt1)
dt3 = gsub("\\.", "/", dt2) #special code for replacing "."
dt4 = unlist(strsplit(dt3, " ") )
date1 = dt4[seq(1, length(dt4), 2)]
...
time2 = as.numeric(substr(time1, 1, 2))
TW = as.matrix(Tw[,-1])
WT.df = cbind.data.frame("Date" = date2, "Time" = time2, TW)

return(WT.df)

}
#WT.df = fncImportTW("data.in/rbm.data/Tw_output.csv")

#Load flow or temperature data for a particular date & time
fncLoadWQdata<- function(type,  dat.df, thedate, thetime, ssn = parent.frame()$ssn, plotit = F){
  
  network = get("network")
  if(network == "rbm") idnm = "arcid"
  if(network == "nhd1") idnm = "pid"
  ...
  dat = cbind(arcID, dat)
  row.names(dat) = NULL
  if(type == "Q") colnames(dat) = c(idnm, q.field)
  if(type == "WT") colnames(dat) = c(idnm, wt.field)
  
  #dat can now be merged to wq.df and has the flow or temperature values for each reach for that date & time
  #note that for network rbm, wq.df has more records than dat because of the reaches we had to split at complex confluences
    #split reaches will both get same prediction value
...
#ssn = fncLoadWQdata(type = "Q", dat.df = Q.df, thedate = "2008-08-15", thetime = 6)
#ssn = fncLoadWQdata(type = "WT", dat.df = WT.df, thedate = "2008-08-15", thetime = 6)
...
fncUnloadWQ<- function(type, ssn = parent.frame()$ssn){
  ...
  if(type == "Q"){
    ...
    if(length(which(colnames(ssn@data) == "Q")) > 0) ssn@data = ssn@data[,-which(colnames(ssn@data) == "Q")]
    if(length(which(colnames(ssn@data) == "Q.color")) > 0) ssn@data = ssn@data[,-which(colnames(ssn@data) == "Q.color")]
  }
  if(type == "WT"){
...
#Load data from one particular date and time (from 'thedata') into the ssn object as "DatTim"
  # for network nhd1 only!
fncLoadDateTime<- function(obj.ssn, thedata, return.ssn = TRUE){
  ssn.df = getSSNdata.frame(obj.ssn)
  R_name=c("Raging.Headwaters","LC.Upper","LC.Beaver","LC.Trib","Raging.Power","Deep.Creek",
           "Raging.PrivateBridge","LC.Lower","Raging.I90","Soderman","Dingford","MF.Dingford",
           "Taylor","MF.DecommissionedRD","Granite","MF.Granite.US","Tolt.Lower","MS.Boatlaunch",
           "NF.Headwaters","Lennox","NF.CountyBridge","Tokul","MS.Lowest","Cherry","MS.Chinook",
           "Griffin","SF.Asahel","SF.Exit38","SF.436Bridge","MF.ThreeForks","NF.Threeforks",
           "MF.MtSi","Icy","Raging.Mouth","Rage.Clust.Mid","Rage.Clust.Up","Tolt.Conf.Tolt",
           "Tolt.Conf.DS","Tolt.Conf.US","Lower.Cherry","MS.Meadowbrook","MS.Above.Raging",
           "Sunday","NF.Sunday","MF.Granite")
  DatTim = rep(NA, length(R_name))
  ssn.df$R_name = R_name
  thesites = intersect(rownames(thedata), R_name)
  for(i in 1:length(thesites)){
    site.name = thesites[i]
    site.idx = which(rownames(thedata) == site.name)
    ssn.index = which(R_name == site.name)
    DatTim[ssn.index] = thedata[site.idx,]
  }
  ssn.df$DatTim = DatTim
  if(return.ssn == TRUE) {
    obj.ssn = putSSNdata.frame(ssn.df, obj.ssn)
    return(obj.ssn)
  } else{
    return(ssn.df)
  }
...
fncGetSO<- function(s, ssn = parent.frame()$ssn) {
  ...
  if(network == "rbm") return(ssn@data[,so.field][ssn@data$rid == s])
  
  if(network == "nhd1") return(wq.df[,so.field][wq.df$rid %in% s][1])
}
...
fncLabelSeg<- function(seg, ssn = parent.frame()$ssn, col = 1, lwd = 1, label = "seg") {
  ...
  #pp = plot(ssn, lwdLineCol = "addfunccol", lwdLineEx = 15, lineCol = "gray50", xlab = "", ylab = "",cex = 0)
  #op = par(usr = pp$usr)
  #for(i in 1:30){ fncLabelSeg(i, label = "so") }
...
fncUseableWidths<- function(dat = parent.frame()$ssn@data[,c("rid", width.field)]){
  widths = dat[,2]
  if(network == "rbm") widths = widths * 1000 #get units to meters
  rid = dat[,1]
...
# Get attribute nearest to a fish
  # 'field' can be "WT", "Q", "ration", "SO" etc. 
fncGetNearestAttribute<- function(dat.df = dat.df[,c("pid", "rid", "ratio", field)],
  ...
  attribs = attribs[order(attribs[,"pid"]),]
  return(attribs)
}
...
fncFishDensity<- function(fish1, fish2, fish1.idx, fish2.idx, ssn = parent.frame()$ssn){
  ...
  if(network == "nhd1") length = length * 1000 # multiply by 1000 because length is in km, width is in m
  width = ssn@data$UseableWidth[ssn@data$accessible ==1]
  area = length * width # area is in m^2
  ...
  for(i in 1:length(rid)){ 
    r = rid[i]
    seg.area = area[i]
    num.fish1.in.seg = sum(fish1[fish1[fish1.idx,"seg"] == r,"survive"]) #only include fish that are alive and emerged
    num.fish2.in.seg = sum(fish2[fish2[fish2.idx,"seg"] == r,"survive"]) #ditto
    density.species1.num[i] = num.fish1.in.seg / seg.area
    density.species2.num[i] = num.fish2.in.seg / seg.area
    biomass.fish1.in.seg = sum(fish1[fish1[fish1.idx,"seg"] == r,"weight"]) #only include fish that are alive and emerged
    biomass.fish2.in.seg = sum(fish2[fish2[fish2.idx,"seg"] == r,"weight"]) #ditto
...
fncMoveDistance<- function(fish = parent.frame()$fish, sp.idx, ssn = parent.frame()$ssn){
  ...
  mvmt.multiplier = parameters[sp.idx, "mvmt.multiplier"]
  mvdst.sdlog = parameters[sp.idx, "mvdst.sdlog"]
  ...
  growths[,"growth"] = fncGrowthFish(fpids, fish)
  
  # look up max growth possible for each fish across all accessible reaches during this time step
  tmp.lst = list()
  tmp.lst$result = lapply(fpids, function(x) fncGrowthPossible(fweight = fish[,"weight"][fish[,"pid"] == x], ssn))
  tmp.range = do.call(rbind,lapply(tmp.lst$result, range))
  growths[,"growth.pot"] = tmp.range[,2]
  growths[,"growth.min"] = tmp.range[,1]

  # #for-loop way is marginally slower:
  # initialtime = proc.time()[3]
  # for(fpid in fpids){
  #   tmp.dat = fncGrowthPossible(fweight = fish[,"weight"][fish[,"pid"] == fpid], ssn)
  #   growths[,"growth.pot"][rownames(growths)==fpid] = max(tmp.dat)
  #   growths[,"growth.min"][rownames(growths)==fpid] = min(tmp.dat)
  # }
  # tm2 = proc.time()[3] - initialtime
  # print(tm2 - tm1)
...
fncStopEarly<- function(fpid, seg = parent.frame()$seg, remainingDist, moveLength, length2segBase, om.prob2 = 1, fish = parent.frame()$fish, ssn = parent.frame()$ssn){
  ...
  result = fncGrowthPossInReach(fish[,"weight"][fish[,"pid"] == fpid], seg = seg)
  ...
  result2 = fncGrowthPossible(fish[,"weight"][fish[,"pid"] == fpid])
...
fncJunctionDecisions<- function(fpid, sp.idx, seg = parent.frame()$seg, currMoveDirection, stochastic = TRUE, fish = parent.frame()$fish, ssn = parent.frame()$ssn) {
  ...
  if(seg == 0){ return(bestSeg = 0)} #can't go farther downstream, this is the base, assume smolts outmigrate
  ...
  if(length(rids) > 1){
    ...
    growth.confluence = fncGrowthPossible(fish[,"weight"][fish[,"pid"] == fpid], ssn, segs = rids)
...
fncMoveIndividual<- function(fpid, fish = parent.frame()$fish, sp.idx, ssn = parent.frame()$ssn) {
    ...
    if(sp.idx == 1) om.prob2 = fncRescale(c(get("om.prob")[fish$pid == fpid],0.5,1), c(0,1))[1]
    if(sp.idx == 2) om.prob2 = 0
    ...
    while(remainingDist > 0 & seg > 0){ #Continue as long as the fish has not yet moved its allocated distance
      ...
      # can happen even if room to move is false, but only happens if fish's drive to outmigrate is less than 100%
      if(om.prob2 < 1){
...
fncGrowthFish<- function(PIDs = NA, fish = parent.frame()$fish){
  ...
  if(SecondSpecies == TRUE) ma.seq = c(1:100,seq(105,995,5), seq(1000,6000,10)) else ma.seq = seq(1, 500, 1) #fish mass
...
#result = fncGrowthFish(4, fish = salmon) #single fish
#result = fncGrowthFish(3:7, fish = fish_other) #vector of fish

# Look up best possible location for growing based on all current temperature/ration across stream network
fncGrowthPossible<- function(fweight = 1, ssn = parent.frame()$ssn, segs = NA){
  ...
  ## Get data, for seg(s) 
  if(any(is.na(segs))){
    td = wq.df[wq.df$rid %in% ssn@data$rid[ssn@data$accessible == 1], c("pid", "rid", "ration", wt.field)]
  } else {
    td = wq.df[wq.df$rid %in% segs & wq.df$rid %in% ssn@data$rid[ssn@data$accessible == 1], c("pid", "rid", "ration", wt.field)]
  }
  row.names(td) = NULL
  
  if(nrow(td) > 0){ #ensure there are data before proceeding
    # for networks that have multiple points per seg:
    if(network == "nhd1") td = aggregate(td, by = list(td[,"rid"]), FUN = mean)[,3:5]
    ...
    ra.idx = apply(td[,"ration", drop = FALSE], 1, function(x) which.min(abs(ra.seq - x)))
...
fncGrowthPossInReach<- function(fweight = 1, ssn = parent.frame()$ssn, seg){
  ...
  ## Get data
  td = wq.df[wq.df$rid %in% seg & wq.df$rid %in% ssn@data$rid[ssn@data$accessible == 1], c("pid", "rid", "ration", wt.field)]
  row.names(td) = NULL
...
#result = fncGrowthPossInReach(100, ssn, seg = 30)
...
fncSurvive<- function(df, minprob = 0.995){
...
# Prey are consumed by predators given that:
# * there are alive predators and alive, emerged prey in the same segment
# * predators are above a temperature threshold
# * prey weight <= (weight threshold) * (predator weight)
...
# probability of predation decreases linearly with the number of prey per segment
# Input:
#   prey.survive = vector of prey survive, length is number of prey fish
#   prey.emerge = vector of prey emrg, length is number of prey fish
#   prey.seg    = vector of prey seg, length is number of prey fish
#   prey.weight = vector of prey weight, length is number of prey fish
#   pred.survive = vector of predator survive, length is number of predator fish
#   pred.temp   = vector of predator WT, length is number of predator fish
#   pred.seg    = vector of predator seg, length is number of predator fish
#   pred.weight = vector of predator weight, length is number of predator fish
#   pred.temp.crit  = minimum threshold for predators to be active
#   pred.mass.crit = ratio of predator to prey weights
#   max.pred.prob   = maximum probability of predator successfully killing prey
# Output:
#   prey.survive = updated vector of prey survive, length is number of prey fish
#      1 represents survival
#      0 represents killed by stochastic element
#     -1 represents killed by predator
fncPredation<- function(prey.survive = salmon$survive,
                     ...
                     pred.mass.crit = 0.5,
                     max.pred.prob = 0.25){
  num.pred = length(pred.seg)
  for (i in 1:num.pred){
    if ((pred.temp[i] >= pred.temp.crit) & (pred.survive[i] == 1)) {
      dist.max = pred.dist[i] + pred.move[i]
      dist.min = pred.dist[i] - pred.move[i]
      potential.prey = which((prey.seg == pred.seg[i]) & 
                             (prey.dist >= dist.min) & (prey.dist <= dist.max) &
                             (prey.emerge == 1) & (prey.survive == 1) &
                             (prey.weight <= pred.mass.crit * pred.weight[i]))
      num.prey = length(potential.prey)
      if (num.prey > 0) {
        s = 1 - max.pred.prob / num.prey
        survivors = rbinom(n = num.prey, size = 1, prob = s)
        survivors = survivors + (survivors == 0) *- 1
        prey.survive[potential.prey] = survivors
      }
    }
  }
  return(prey.survive)
...
fncGetParametersSA<- function(vary_nFish = FALSE){
  ...
  salmon.parm.list = c("nSpawnDays", "ATU.crit", "mvmt.multiplier", "ration.hi", "maxDensity4Growth", "smolt.mass")
  for(parm in salmon.parm.list){parameters["salmon",parm] = rnorm(n = 1, mean = parameters["salmon",parm], sd = 0.125 * parameters["salmon",parm])}
  parameters["salmon","min.survive.prob"] = sample(seq(0.9,0.999,0.001),1)
  parameters[,"nSpawnDays"] = round(parameters[,"nSpawnDays"],0)
  parameters["salmon","spawn.firstdate"] = paste0("09-", sprintf("%02d", sample(seq(1:20),1)))
  a = paste0("06-", sprintf("%02d", sample(seq(20:30),1))); b = paste0("07-", sprintf("%02d", sample(seq(1:10),1)))
  parameters["salmon","smolt.date.cutoff"] = sample(c(a,b),1)
  
  if(SecondSpecies == TRUE){
    fish_other.parm.list = c("ration.hi", "mvmt.multiplier", "maxDensity4Growth", "max.pred.prob")
    for(parm in fish_other.parm.list){parameters["fish_other",parm] = rnorm(n = 1, mean = parameters["fish_other",parm], sd = 0.125 * parameters["fish_other",parm])}
    parameters["fish_other","min.survive.prob"] = sample(seq(0.9,0.999,0.001),1)
  }
  if(vary_nFish == TRUE){
    parameters["salmon","nFish"] = rnorm(n = 1, mean = parameters["salmon","nFish"], sd = 0.125 * parameters["salmon","nFish"])
    parameters["fish_other","nFish"] = rnorm(n = 1, mean = parameters["fish_other","nFish"], sd = 0.125 * parameters["fish_other","nFish"])
  }
  return(parameters)
}
