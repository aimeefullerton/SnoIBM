...
for(scenario in scenario.list){
  ...
  for(yy in years){
    ...
    load(file = paste0("/Volumes/BluPassport/SnoIBM/data.out_riparian0/", outdir, "/salmon.finalstep.", yy, ".", iter, ".RData"))
...
for(scenario in scenario.list){
  for(metric in c("emergence", "outmigration", "subyearlings", "yearlings")){
      ...
      #Min/Max
      polygon(c(minX: maxX, rev(minX: maxX)), c(dat[minX: maxX, "0%"], rev(dat[minX: maxX, "100%"])), border = NA, col = mycolors[[1]][4])
      
      #Q1/Q3
      polygon(c(minX: maxX, rev(minX: maxX)), c(dat[minX: maxX, "25%"], rev(dat[minX: maxX, "75%"])), border = NA, col = mycolors[[2]][4])
      
      #Median
      lines(minX:maxX, dat[minX: maxX,"50%"], lwd = 2, col = mycolors[[3]][4])
