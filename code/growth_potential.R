...
for(salmon.nm in c("chinook", "coho", "steelhead", "pink", "lmb", "rainbow")){
  for(scenario in c("bcc-csm1-1-m", "CanESM2", "CCSM4", "CNRM-CM5", "CSIRO-Mk3-6-0", "HadGEM2-CC365", "HadGEM2-ES365", "IPSL-CM5A-MR", "MIROC5", "NorESM1-M")){
    ...
    #=== INITIALIZE FISH ===========================================================
      ...
      # Salmon
      shp <- readOGR(paste0(mydir,"/",loadDir,"/", ssn.folder), "preds")
      ...
      writeOGR(shp, paste0(mydir,"/",loadDir,"/", ssn.folder), "accessible_reaches", driver = "ESRI Shapefile", overwrite_layer = T)
    ...
    #=== RUN SIMULATION ============================================================
      ...
      # 3. PLOT MAPS ====
          ...
          if(plot.GP){
            ...
            png(paste0(mydir, "/", imageDir, "/GrwPot/GrwPot_", dat.idx[dd], "-", sprintf("%03d", tt), ".png"), width = 9, height = 6, units = "in", res = 150)
            par(mar = c(1, 0, 4, 0))
    ...
    for(s in 1:5){
      ...
      png(paste0(mydir, "/", imageDir, "/GrwPot_", gr.nam, ".png"), width = 9, height = 6, units = "in", res = 150)
      par(mar = c(1, 0, 4, 0))
    ...
    for(s in 1:5){
      ...
      for(k in 1:5){
        ...
        png(paste0(mydir, "/", imageDir, "/WT_", wt.nam, ".png"), width = 9, height = 6, units = "in", res = 150)
        par(mar = c(1, 0, 4, 0))
