...
rm(list=ls()); gc() #clear workspace before beginning to reduce the chance of getting irreproducible results
...
#=== B. INITIALIZE HABITAT ========================================================
  ...
  Q.RI.benchmarks <- read.csv("data.in/parameters/Q_bm.csv", stringsAsFactors = FALSE)
    ...
    #ri.vars <- NULL; for(ri in Q.RI.benchmarks$RI) ri.vars <- c(ri.vars, paste0("Q",ri))
  ssn <- fncLoadQ_RI()
...
#=== D. RUN SIMULATION ============================================================
  ...
  # 1. UPDATE HABITAT: Get predicted stream temperatures and flow ====
      ...
      if(plot.flag == TRUE & iter == plot.iter){
        ...
        png(paste0(mydir, "/", imageDir, "/WT", dat.idx[dd], "-", sprintf("%03d", tt), ".png"), width = 9, height = 6, units = "in", res = 150)
        par(mar = c(1, 0, 4, 0))
        ...
        if(network == "rbm"){
        png(paste0(mydir, "/", imageDir, "/Q", dat.idx[dd], "-", sprintf("%03d", tt), ".png"), width = 9, height = 6, units = "in", res = 150)
          par(mar = c(1, 0, 4, 0))
          # plot background
          if (outline) {
            plot(basin,col = "gray80", border = NA)
