...
#   to altered thermal and flow regimes [and optionally to a second species]  
...
#    Last functional update 21 Apr 2020; minor editorial updates 23 Oct 2020
#
# Impassable barriers:
...
# Instructions. Choose scenario options here before running simulations. Choose either a set from 'Calibration' or a set from 'Scenarios'
# and run those lines of code. Then collapse the bracket associated with "II. Run Scenario Simulations", highlight it and run all remaining lines.
# Use "Code/JumpTo", the scrollbox picker at the bottom left of this pane, or the view document outline button at the top right of this pane
# in order to move quickly among code sections.
...
years2run <- 2006:2013 #Baseline (all one year less b/c water quality time series start on 10-01 whereas fish model starts on 09-01)
...
climate.scenario <- "bcc-csm1-1-m"# or choose from list above
riparian.scenario <- "riparian0" # or choose from list above

#=== II. RUN SCENARIO SIMULATIONS ==================================================
...
# Global Variables
  ...
  ifelse(interspecific.predation.flag == TRUE, tag <- "P", tag <- "A") #Predation or Alone
  ...
  iter <- 1
  plot.iter <- 99 # which iteration will have maps plotted for each time step
  run <- fncGetRun()
...
# Plotting
  ...
  if (plot.flag == TRUE) {
    ...
    left2 <- seq(0, 2500, length.out = 9)[1:8]
    rght2 <- seq(0, 2500, length.out = 9)[2:9]
...
# Load model parameters
  ...
  spawner.estimates <- read.csv(paste0(loadDir, "/parameters/nSpawnersByYear_WDFW.csv"), stringsAsFactors = FALSE, header = TRUE)
...
# Load pre-calculated growth for each temperature/ration/size combination 
  ...
  load(paste0(loadDir, "/fish.growth.lookup/wt.growth.array.", parameters["salmon","species"], ".RData"))
  assign("wt.growth.salmon", wt.growth)
  if(SecondSpecies == TRUE){
    load(paste0(loadDir, "/fish.growth.lookup/wt.growth.array.", parameters["fish_other","species"], ".RData"))
...
# Start looping through iterations (if more than one)
for(iter in iter.list){
  ...
  if(yy %in% spawner.estimates$Year){ # when there are year-specific data from spawner surveys, use it
    nFish <- spawner.estimates$nSpawners[spawner.estimates$Year == yy] / 2.5 * parameters["salmon","eggs.per.redd"] / parameters["salmon","corr.factor"] 
      # where 2.5 represents the no. of adults per redd on average
...
#=== B. INITIALIZE HABITAT ========================================================

  # Load the Snoqualmie Spatial Stream Network (SSN)
  if(network == "rbm"){ #if using the DHSVM-RBM stream hydrography
  ...
  # Add column to denote which reaches are accessible to fish (for blocking movement) based on StreamNet.org distributions
...
#=== C. INITIALIZE FISH ===========================================================
  ...
  if(SecondSpecies == TRUE){
    ...
    # add fish locations to plot
    #points(ssn@predpoints@SSNPoints[[fish_other.id]]@point.coords[,1], ssn@predpoints@SSNPoints[[fish_other.id]]@point.coords[,2], col = 2, pch = 1, cex = 0.5)
  ...
  # spawning window is based on empirical data from WDFW spawner surveys 2005-2015.
  ...
  # the first element is the number of salmon that will spawn on the first timestep of spawn.init
  set.seed(yy + iter)
  salmon.spawn.times <- histsu(rnorm(nrow(salmon), mean = 0, sd = parameters["salmon", "spawn.date.shape"]), 
                        breaks = (parameters["salmon", "nSpawnDays"] * 2), plot = FALSE)$counts
  # sum(salmon.spawn.times); length(salmon.spawn.times); plot(salmon.spawn.times)

  # Add flood recurrence interval flow fields to salmon dataframe
  if(network == "rbm"){
    for(ri in ri.vars){
      salmon[,ri] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", ri)], salmon[salmon$survive == 1, c("pid", "seg", "ratio")], ssn)[,ri]
    }
  }
...
#=== D. RUN SIMULATION ============================================================
  ...
  for(dd in 1:length(dat.idx)){
    for(tt in c(6, 18)){ #6am, 6pm
      ...
      # Print current run information to the screen
      ...
      if(show.progress == TRUE){
        ...
        cat("proportion yearlings: ", length(salmon$weight[salmon$survive == 1]) / (length(salmon$weight[salmon$survive == 2]) + 
            length(salmon$weight[salmon$survive == 1])), "\n")
  ...
  # 1. UPDATE HABITAT: Get predicted stream temperatures and flow ====
      ...
      if(plot.flag == TRUE & iter == plot.iter){
        ...
        png(paste0(imageDir, "/WT", dat.idx[dd], "-", sprintf("%03d", tt), ".png"), width = 9, height = 6, units = "in", res = 150)
        par(mar = c(1, 0, 4, 0))
        ...
        if(network == "nhd1"){ #aggregate points on a stream reach
        ...
        points(ssn@predpoints@SSNPoints[[salmon.id]]@point.coords[,1][ssn@predpoints@SSNPoints[[salmon.id]]@point.data$pid %in% open.salmon],
              ...
              col = 1, pch = 1, cex =0.5)
        #points(salmon$xloc[salmon$pid %in% open.salmon], salmon$yloc[salmon$pid %in% open.salmon])
        ...
        #points(salmon$xloc[salmon$pid %in% spawned.salmon], salmon$yloc[salmon$pid %in% spawned.salmon])
        ...
        #points(salmon$xloc[salmon$pid %in% emerged.salmon], salmon$yloc[salmon$pid %in% emerged.salmon])
        ...
        # plot flow for dd & tt as colored stream lines:
        if(network == "rbm"){
        png(paste0(imageDir, "/Q", dat.idx[dd], "-", sprintf("%03d", tt), ".png"), width = 9, height = 6, units = "in", res = 150)
          par(mar = c(1, 0, 4, 0))
          # plot background
          if (outline) {
            plot(basin,col = "gray80", border = NA)
          ...
          for (i in 1:length(ssn@lines)) {
            for (j in 1:length(ssn@lines[[i]])) {
              lines(ssn@lines[[i]]@Lines[[j]]@coords, col = cb2[ssn@data[i,"Q.color"]], lwd = 10 * (ssn@data[i, "afvArea"] + 0.1))
            }
          }
          
          # Add legend
          leglabs <- paste(left2, "to", rght2)
          legend("right", legend = leglabs, title = "Flow (cms)", bty = "n", pch = 19, col = cb2, cex = 0.8)
  ...
  # 3. INCUBATE EGGS ====
      ...
      salmon$WT[salmon$survive == 1] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", wt.field)], 
               salmon[salmon$survive == 1, c("pid", "seg", "ratio")], ssn)[,wt.field]
      
      # Accumulate thermal units (TUs) While salmon are spawned or emerged
      salmon$TU[salmon$survive == 1 & salmon$emrg != 0] <- salmon$TU[salmon$survive == 1  & salmon$emrg != 0] + 
               salmon$WT[salmon$survive == 1  & salmon$emrg != 0] * 0.5 #halved to account for half-day time step
      
      # Assign closest flow to salmon
      if(network == "rbm") salmon$Q[salmon$survive == 1] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", q.field)], 
                salmon[salmon$survive == 1, c("pid", "seg", "ratio")], ssn)[,q.field]
      
      # Assign closest WT & Q and accumulate TU for a second species, if present
      if(SecondSpecies == TRUE){
        fish_other$WT[fish_other$survive == 1] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", wt.field)], 
                  fish_other[fish_other$survive == 1, c("pid", "seg", "ratio")], ssn)[,wt.field]
        fish_other$TU[fish_other$survive == 1] <- fish_other$TU[fish_other$survive == 1] + 
                  fish_other$WT[fish_other$survive == 1] * 0.5 #halved to account for half-day time step
        fish_other$Q[fish_other$survive == 1] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", q.field)], 
                  fish_other[fish_other$survive == 1, c("pid", "seg", "ratio")], ssn)[,q.field]
      }
      
      # Scour redds if peak flows are large enough
      if(network == "rbm"){
        set.seed(yy + iter) #set reproducible seed
        
        # Kill eggs based on flow recurrence interval
        ...
        # Record the date eggs died
        salmon$dateDi[salmon$dateDi == day1 & salmon$survive == -2] <- strptime(dat.idx[dd], format = "%Y-%m-%d")
      }
    
      # Emerge salmon alevins
      if (any(salmon$emrg == -1)) {
        # Alevins after they have reached an accumulated thermal unit (TU) threshold
        salmon$emrg[salmon$emrg == -1 & salmon$survive == 1 & salmon$TU >= parameters["salmon", "ATU.crit"]] <- 1
        
        # Record the date alevins emerged
        salmon$dateEm[salmon$dateEm == day1 & salmon$emrg == 1] <- strptime(dat.idx[dd], format="%Y-%m-%d")
      }
      
      
  # 4. MOVE FISH ====
      # After emergence, assess probability of movement and move fish individually if appropriate
      ...
      if(length(salmon.alive.emerge.index) > 0){
        ...
        set.seed(yy + iter)
        om.prob <- fncDownstreamDrive(w = salmon[salmon.alive.emerge.index, "weight"], om.mass = parameters["salmon", "om.mass"], 
                   om.date.taper = parameters["salmon", "om.date.taper"])
      ...
      if(SecondSpecies == TRUE){
        ...
        if(dd == 1 & tt == 6){
          fish_other[fish_other.alive.index,] <- fncJitterPosition(wt.growth <- wt.growth.fish_other, 
              fae = fish_other.alive.index, fish = fish_other[fish_other.alive.index,], sp.idx = 2) #, plotit = "screen")
        }
  ...
  # 5. RE-ASSESS HABITAT QUALITY (to incorporate changes after movement into growth calcs) ====
        
      # Get nearest water temperature (& flow)
      ...
      if(network == "rbm"){
        salmon$Q[salmon.alive.emerge.index] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", q.field)], 
                                               salmon[salmon.alive.emerge.index,c("pid","seg","ratio")], ssn)[,q.field] 
      }
      }
      
      if(SecondSpecies == TRUE){
        if(length(fish_other.alive.index) > 0){
          fish_other$WT[fish_other.alive.index] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", wt.field)], 
                                                   fish_other[fish_other.alive.index,c("pid","seg","ratio")], ssn)[,wt.field] 
          if(network == "rbm"){
            fish_other$Q[fish_other.alive.index] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", q.field)], 
                                                    fish_other[fish_other.alive.index,c("pid","seg","ratio")], ssn)[,q.field] 
          }
        }
      }
        
      # Update fish densities to incorporate fish that just moved into density estimates
      if(length(salmon.alive.emerge.index) > 0){
      ...
      # Update available ration after fish have moved and after accounting for density
      ...
      if(length(salmon.alive.emerge.index) > 0){
        ...
        #remove the last 2 values that were used to standardize the range:
        density.effect <- density.effect[-c(length(density.effect), (length(density.effect) -1 ))] 
      ...
      if(SecondSpecies == TRUE){
        if(length(fish_other.alive.index) > 0){
          fish_other$ration[fish_other.alive.index] <- fncGetNearestAttribute(wq.df[,c("pid", "rid", "ratio", "ration_ss")], 
                                                       fish_other[fish_other.alive.index,c("pid","seg","ratio")], ssn)[,"ration_ss"] 
        }
      }
         
  # 6. GROW FISH. (calculated using the Wisconsin Bioenergetics model equations) ====
      ...
      if(SecondSpecies == TRUE){
        if(length(fish_other.alive.index) > 0){
          ...
          # Mark any fish_other with negative weights as dead and reset index
  ...
  # 8. OTHER MORTALITY
      ...
      salmon$survive[salmon.alive.emerge.index] <- fncSurvive(df = salmon[salmon.alive.emerge.index, c("weight","growth")], 
          minprob = parameters["salmon","survival.min"], maxprob = parameters["salmon","survival.max"], b = parameters["salmon","survival.shape"])
      salmon.alive.emerge.index <- salmon$pid[salmon$emrg == 1 & salmon$survive == 1]
      ...
      if(SecondSpecies == TRUE){
        if(length(fish_other.alive.index) > 0){
          fish_other$survive[fish_other.alive.index] <- fncSurvive(df = fish_other[fish_other.alive.index, c("weight","growth")], 
              minprob = parameters["fish_other","survival.min"], maxprob = parameters["fish_other","survival.max"], b = parameters["fish_other","survival.shape"])
          fish_other.alive.index <- fish_other$pid[fish_other$survive ==1]
        }
        # Record the date fish died
