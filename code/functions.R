...
#    Last functional update 26 Apr 2020; minor editorial updates 23 Oct 2020
#
#*******************************************************************************
...
fncImportQ <- function(filepath){
  ...
  # get 'arcid' header row from DHSVM file
  ids = as.numeric(unlist(Qr[1,-1]))
  # translate 'arcid' into 'rid' column for working with SSN
  namedvec <- ssn@data$rid; names(namedvec) = paste0("X", ssn@data$arcid)
  new.ids <- fncGetValue(paste0("X", ids), namedvec)
  colnames(Qr) <- c("Date", paste0("X",new.ids))
  Qr <- Qr[-1,]
...
# Read in and format water temperature predictions from RBM
Tw <- read.csv(filepath, skip = 1, header = FALSE, stringsAsFactors = FALSE)
# get 'arcid' header row from DHSVM file
ids <- as.numeric(unlist(Tw[1,-1]))
# translate 'arcid' into 'rid' column for working with SSN
namedvec <- ssn@data$rid; names(namedvec) = paste0("X", ssn@data$arcid)
new.ids <- fncGetValue(paste0("X", ids), namedvec)
colnames(Tw) <- c("Date", paste0("X", new.ids))
Tw <- Tw[-1,]
...
fncLoadWQdata <- function(type,  dat.df, thedate, thetime, ssn = parent.frame()$ssn, plotit = F){
  ...
  obsdat <- obsdat[order(obsdat$pid),]
  row.names(obsdat) = obsdat$pid #super important step: needs to be ordered by pid and row.names = pid
...
# Load flood recurrence interval data into SSN
fncLoadQ_RI <- function(dat.df = Q.RI.benchmarks, ssn = parent.frame()$ssn){
  # requires dat.df = Q.RI.benchmarks to be a Q.df that has the correct benchmark dates in it (could be the whole time series or just these dates, either is fine)
  # calculated in 'Flood.RI.Benchmarks.R'
  ...
  for(ri in Q.RI.benchmarks$RI){
    thedate <- Q.RI.benchmarks$Date[Q.RI.benchmarks$RI == ri]
    thetime <- Q.RI.benchmarks$Time[Q.RI.benchmarks$RI == ri]
    
    id.values <- as.numeric(gsub("X", "", colnames(dat.df)[-c(1:3)]))
    dat <- t(dat.df[dat.df[,"Date"] == as.Date(thedate) & dat.df[,"Time"] == thetime, 4:ncol(dat.df)])
    dat <- cbind(id.values, dat[,1]) #get first column (could be more because there may be multiple RIs that have the same date)
    row.names(dat) <- NULL
    colnames(dat) <- c(idnm, paste0("Q", ri))
...
fncFishShp <- function(species, nSpawners, nStart, ssn = parent.frame()$ssn, plotit = FALSE){
  ...
  shp <- readOGR(paste0(loadDir,"/", ssn.folder), shpnm2)
  ...
  writeOGR(fish_spawn, paste0(loadDir, "/", ssn.folder), substr(shpnm, 1, nchar(shpnm) - 1), driver = "ESRI Shapefile", overwrite_layer = T)
  #writeOGR(fish_spawn, paste0(loadDir, "/", ssn.folder), paste0(shpnm, nSpawners), driver = "ESRI Shapefile", overwrite_layer = T)
...
# Load data from one particular date and time (from 'thedata') into the ssn object as "DatTim"
  # for network nhd1 and Snoqualmie watershed only!
...
# Moving average function for summarizing results
# x: the vector
# n: the number of samples
# centered: if FALSE, then average current sample and previous (n-1) samples
#           if TRUE, then average symmetrically in past and future. (If n is even, use one more sample from future.)
fncMvgAvg <- function(x, n=1, centered=FALSE) {
...
fncUpdatePosition <- function(newSeg, seg = parent.frame()$seg, sp.idx, moveDirection, length2segBase, dist2move, remainingDist, 
  ...
  segRatio <- fncLength2segRatio(newSeg, length2segBase)
...
fncGrowthPossible <- function(fweight = 1, sp.idx = 1, ssn = parent.frame()$ssn, segs = NA){
  ...
  if(nrow(td) > 0){ #ensure there are data before proceeding
    td <- aggregate(td, by = list(td[,"rid"]), FUN = mean)[,3:5] #aggregate all points in a reach
    if(any(is.na(td[,wt.field]))){# remove any NA records
      td <- td[-which(is.na(td[,wt.field])),]
    }
...
fncPeakFlowSurvival <- function(fish, aa = parameters["salmon","pk.q.a"], bb = parameters["salmon","pk.q.b"], adj = parameters["salmon","pk.q.adj"]){
  ...
  # (at Mt. Vernon) in the Skagit River basin (1989 to 1996) (y = 0.1285 e -0.0446x, R2 = 0.97).
  # Seiler et al. 1998: the data used in Beamer and Pess, and they fit linear relationship with flow (cfs) directly.
  # See Figure 14 in Kinsel et al. 2007 WDFW
  ...
  # aa <- 1 # Making max survival for given time step 1 because fish are going to experience a given flood for more than one time step and there may be other floods too. 
  # Will have to calibrate bb (and maybe aa):
  bb <- bb * adj
  
  # For each redd at each time step, assign a "survival probability" for eggs based on flows for current date/time as compared to approx. bankfull flow (FRI of 1.01 years)
  # Units for DHSVM flow = cubic meters per second
  
  # Translate flow at fish's location (fish$Q) into the proportion of bankfull flow (p.bff) and then translate that into a flood recurrence interval (fri)
  p.bff <- fish$Q / fish$Q1.25 #proportion of flow at fish's location this time step (fish$Q) relative to bankfull flow (Q1.25) at that location
  fri <- 0.3182 * exp(1.376 * p.bff) #exponential equation for translating the flow multiplier from previous step into a flood recurrence interval; see 'Flood.RI.Benchmarks.R'
  probs <- aa * exp(bb * fri)  #exponential equation for translating flood recurrence interval into egg to fry survival
  ...
  survivors[survivors == 0] <- -2 # assign the code unique to eggs dying from being scoured
  
  return(survivors)
}

# Generalized size-based survival for juveniles
